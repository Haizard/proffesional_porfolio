...
export default function AccountContent({ user, profile }: Props) {
      ...
      <div className="container mx-auto px-4 max-w-5xl">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-end mb-12 gap-4">
          ...
          <Button
            ...
            className="font-mono text-red-500 hover:text-red-500 hover:bg-red-500/10 border-red-500/30"
          >
              ...
              <CardContent className="space-y-4">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    {profile?.role && (
                      <span className="text-[10px] uppercase font-mono text-primary tracking-wider">
                        {profile.role}
                      </span>
                    )}
                  </div>
          ...
          <div className="md:col-span-2 space-y-8">
            <Card className="bg-card border-border">
              <CardHeader className="flex flex-row items-center justify-between">
                ...
                <Link href="/account/orders" className="text-xs font-mono text-primary hover:underline">
                  VIEW_ALL
                </Link>
              </CardHeader>
              <CardContent>
                {orders.length > 0 ? (
                  <div className="space-y-4">
                    {orders.slice(0, 3).map(order => (
                      <div
                        key={order.id}
                        className="flex items-center justify-between p-4 border border-border rounded-sm bg-background/50"
                      >
                        <div className="flex items-center gap-4">
